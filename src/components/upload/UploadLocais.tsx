...
import { uploadLocaisTrabalho, type UploadResult } from '@/lib/actions/upload.actions';
import { gerarTemplateLocais } from '@/lib/utils/excel-templates';
import { Button } from '@/components/ui/Button';
import { Upload, Download, FileText, CheckCircle, XCircle } from 'lucide-react';

export default function UploadLocais() {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      const response = await uploadLocaisTrabalho(file);
  ...
  const downloadTemplate = () => {
    const buffer = gerarTemplateLocais();
    const blob = new Blob([buffer], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'template_locais.xlsx';
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Upload de Locais de Trabalho</h2>
            <p className="text-gray-600 mt-1">Faça upload em massa de locais de trabalho via Excel</p>
          </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              <tr className="border-b">
                ...
                <td className="py-2">Sede principal</td>
              </tr>
            </tbody>
          </table>
        </div>
