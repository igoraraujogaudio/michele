-- ============================================================================
-- FIX EMERGENCIAL v2: Recriar views para relatórios funcionarem
-- ============================================================================
-- Execute este arquivo SQL diretamente no Supabase SQL Editor
-- ============================================================================

-- 1. Remover views existentes (se existirem)
DROP VIEW IF EXISTS vw_veiculos_em_manutencao CASCADE;
DROP VIEW IF EXISTS vw_veiculos_disponiveis CASCADE;
DROP VIEW IF EXISTS vw_veiculos_completos CASCADE;

-- 2. VIEW EM MANUTENÇÃO (versão corrigida)
CREATE OR REPLACE VIEW vw_veiculos_em_manutencao AS
SELECT 
  v.id AS veiculo_id,
  COALESCE(p.nome, 'SEM PREFIXO')::character varying AS prefixo,
  v.placa,
  v.modelo,
  COALESCE(lt.nome, 'SEM LOCAL')::character varying AS local_trabalho,
  v.nome_motorista,
...
-- 3. VIEW DISPONÍVEIS (versão corrigida)
...
SELECT 
  ...
  v.nome_motorista,
  v.telefone_motorista,
  'DISPONÍVEL'::character varying AS status_atual,
  NULL::character varying AS ultima_ordem_numero,
  NULL::character varying AS ultima_ordem_status,
  NULL::timestamp with time zone AS ultima_ordem_data_fechamento,
  NULL::integer AS ultima_ordem_tempo_parado,
  NULL::numeric AS dias_desde_ultima_manutencao,
...
BEGIN
  ...
  RAISE NOTICE '🚀 Views recriadas! Relatórios devem funcionar agora.';
END $$;
